...
def main():
    ...
    while True:
        ...
        if (x == tekix*10) and (y == tekiy*10): #敵をぶつかったら、ゲームオーバー
            font = pg.font.Font(None, 30) 
            text1 = font.render("Game Over!", True, color_red)
            text2 = font.render(f"Score {scor}", True, color_red)   # 描画する文字列の設定
            scrn_sfc.blit(text1, [300, 200])# 文字列の表示位置
            scrn_sfc.blit(text2, [400, 200])
            sys.exit()
        else:
